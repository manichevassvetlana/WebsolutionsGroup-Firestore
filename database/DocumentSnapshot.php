...
class DocumentSnapshot extends \Google\Cloud\Firestore\DocumentSnapshot implements \Serializable
{
    ...
    public static function create($settable, $autoGenerated = true)
    {
        $class = get_called_class();
        $class = new $class();
        if (is_null($class->collection)) return null;
        $fillable = [];
        foreach ($class->fillable as $key => $field) {
            if (is_array($field)) {
                $isArray = array_key_exists($key, $settable) && is_array($settable[$key]);
                $fillable[$key] = $isArray ? $settable[$key] : [];
                foreach ($field as $objectField) {
                    $fillable[$key][$objectField] = $isArray && array_key_exists($objectField, $settable[$key]) ? $settable[$key][$objectField] : null;
                }
            } else $fillable[$field] = array_key_exists($field, $settable) ? $settable[$field] : null;
        }
        $isAuto = $autoGenerated && !array_key_exists("id", $settable);
        $doc = $isAuto ? $class->collection->add($fillable) : $class->collection->document($settable['id'])->set($fillable);
        return $isAuto ? $doc->snapshot() : $class->findOrFail($settable['id']);
    }
    ...
    public function update(array $settable)
    {
        ...
        foreach ($this->fillable as $key => $field) {
            if (!is_array($field)) { if (array_key_exists($field, $settable)) $fillable[$field] = $settable[$field]; }
            else {
                if (array_key_exists($key, $settable) && is_array($settable[$key])) {
                    foreach ($field as $objectField){
                        if (array_key_exists($objectField, $settable[$key])) $fillable[$key][$objectField] = $settable[$key][$objectField];
                    }
                }
            }
        }
