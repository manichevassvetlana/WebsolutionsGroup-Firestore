...
class Query extends \Google\Cloud\Firestore\Query
{
    use DebugInfoTrait;
    use SnapshotTrait;
    ...
    public function documents(array $options = [])
    {
        ...
        //dd(get_class(new QuerySnapshot($this, $rows)));
     ...
     */
    public function where($fieldPath, $operator, $value = null)
    {
        if (is_null($value)) {
            $value = $operator;
            $operator = '=';
        }
        if (strtolower($operator) == 'like') $response = $this->orderBy($fieldPath)->startAt([$value])->endAt(['\uf8ff']);
        else $response = $this->whereRealization($fieldPath, $operator, $value);
        return $response;
    }

    private function whereRealization($fieldPath, $operator, $value){
